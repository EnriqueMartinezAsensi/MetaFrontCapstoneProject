import { StyledCircle, StyledPolyline } from "./BackIcon.styled";

const BackIcon = () => {
  return (
    <svg width='62px' height='62px' viewBox='0 0 512 512' xmlns='http://www.w3.org/2000/svg'>
      <g data-name='Layer 2' id='Layer_2'>
        <g
          data-name='E416, back, Media, media player, multimedia, player'
          id='E416_back_Media_media_player_multimedia_player'
        >
          <StyledCircle cx='256' cy='256' r='246' />
          <StyledPolyline points='333.82 100.37 178.18 256 333.82 411.63' />
        </g>
      </g>
    </svg>
  );
};

export default BackIcon;